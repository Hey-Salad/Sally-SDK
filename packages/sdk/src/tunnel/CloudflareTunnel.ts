...
import { TunnelRegistry, type TunnelRegistration } from "./TunnelRegistry.js";
...
export class CloudflareTunnel {
  ...
  private readonly cloudflaredCommand: string;
  private readonly lifecycle: EventEmitter;
  private readonly mode: TunnelMode;
  private readonly quickTunnelTimeoutMs: number;
  private registry: TunnelRegistry | undefined;
  ...
  private sessionMode: Exclude<TunnelMode, "auto"> | undefined;
  ...
  constructor(options: CloudflareTunnelOptions = {}) {
    ...
    this.lifecycle = options.lifecycle ?? (process as unknown as EventEmitter);
    this.mode = options.mode ?? "auto";
    this.quickTunnelTimeoutMs = options.quickTunnelTimeoutMs ?? 15_000;
    this.registry = options.registry;
  }

  async open(options: OpenTunnelOptions): Promise<TunnelSession> {
    if (this.session) {
      return this.session;
    }
  ...
  private async openNamedTunnel(options: OpenTunnelOptions): Promise<TunnelSession> {
  ...
  private async openQuickTunnel(options: OpenTunnelOptions): Promise<TunnelSession> {
  ...
  private async createTunnel(name: string): Promise<string> {
  ...
  private async register(session: TunnelSession): Promise<void> {
  ...
  private async deregister(session: TunnelSession): Promise<void> {
    const registry = this.getRegistry();
    if (!registry) {
      return;
    }
  ...
  private getRegistry(): TunnelRegistry | undefined {
    if (this.registry) {
  ...
  private resolveMode(options: OpenTunnelOptions): Exclude<TunnelMode, "auto"> {
  ...
  private toRegistration(session: TunnelSession): TunnelRegistration {
    return {
...
function parseTunnelId(output: string): string | undefined {
